...
[publisher ↔ subscriber correlation](/docs/observer-js/sfu/) possible at all — the resolver reads
`producerId` / `consumerId` out of track attachments.
...
| npm: core types | the sample-schemas package |
| npm: binary encoding | `@observertc/samples-encoder` *(deprecated in [3.4.0](../v3-4-0/))* |
| npm: binary decoding | `@observertc/samples-decoder` *(deprecated in [3.4.0](../v3-4-0/))* |
