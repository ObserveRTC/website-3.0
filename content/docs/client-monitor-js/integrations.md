...
If you are going to feed samples to [`observer-js`](/docs/observer-js/) and want it to
...
freeze?"* answerable. See [Remote track resolution](/docs/observer-js/sfu/).
...
{{< callout context="caution" title="There is no global setLogger()" icon="alert-triangle" >}}
It was removed in 4.3.0 in favour of per-instance injection through the constructor. If you were
calling `setLogger`, move that object into `new ClientMonitor({ logger })`.
{{< /callout >}}
...
|---|---|---|
| `ChromeStatsAdapter` | Chrome, Edge, Opera | Chromium-specific field shapes and omissions |
| `FirefoxStatsAdapter` | Firefox | Normalises `mediaType` to `kind`; reconstructs a transport report from the selected candidate pair before 153; restores `transportId` where it is omitted |
| `SafariStatsAdapter` | Safari | WebKit field shapes, and the fields WebKit does not fill at all |
| mediasoup probator filter | mediasoup sources | Drops `probator` track records |

An adapter **never invents a measurement**: where a browser reports nothing, the field stays absent
and whatever reads it says so through `inputsUnavailable` rather than reading as healthy.
